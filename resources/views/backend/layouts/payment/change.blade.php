...
<div class="se--main-layout">
    ...
    <div class="page-title">Change Subscription Details</div>
    <form action="{{ route('admin.subscription.update') }}" method="post">
        @csrf
        <label for="">Name</label>
        <div class="gap-3 px-3 py-2 bg-black border rounded ak-w-34 d-flex justify-content-between border-dark">
            <input type="text" name="name" placeholder="Enter Name" value="{{ $subscription->name ?? '' }}"
                class="bg-black text-white border-0 @error('name') is-invalid @enderror">
            @error('name')
            <span class="invalid-feedback" role="alert">
                <strong>{{ $message }}</strong>
            </span>
        ...
        <div class="gap-3 px-3 py-2 bg-black border rounded ak-w-34 d-flex justify-content-between border-dark">
            <input type="text" name="price" placeholder="Enter Price" value="{{ $subscription->price ?? '' }}"
                class="bg-black text-white border-0 @error('price') is-invalid @enderror">
        ...
        <br>
        <label for="">Stripe Product ID</label>
        <div class="gap-3 px-3 py-2 bg-black border rounded ak-w-34 d-flex justify-content-between border-dark">
            <input type="text" name="stripe_product_id" placeholder="Enter Stripe Subscription ID"
                value="{{ $subscription->stripe_subscription_id ?? '' }}"
                class="bg-black text-white border-0 @error('stripe_product_id') is-invalid @enderror">
        ...
        <div class="gap-3 px-3 py-2 bg-black border rounded ak-w-34 d-flex justify-content-between border-dark">
            <input type="text" name="stripe_price_id" placeholder="Enter Stripe Price ID" readonly
                value="{{ $subscription->stripe_price_id ?? '' }}"
                class="bg-black text-white border-0 @error('Stripe stripe_price_id ID') is-invalid @enderror">
        ...
        <button type="submit" class="btn btn-light fixed-width">Change Subscription Content</button>
    </form>
